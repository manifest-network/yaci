...
const TotalPayoutBurnQuery = `
		...
		payout_total AS (
          WITH payout_events AS (
			SELECT
		  	e.id,
		  	e.event_index,
		  	MAX(CASE WHEN e.attr_key = 'payout_amount' THEN e.attr_value END) AS amount_raw,
		  	MAX(e.msg_index) AS msg_index
		    FROM api.events_main e
		    WHERE e.event_type = 'payout'
		    GROUP BY e.id, e.event_index
		  )
		  SELECT COALESCE(SUM((rm.captures)[1]::numeric), 0) AS amount
		  FROM payout_events b
		  JOIN api.messages_main m
		    ON m.id = b.id
		   AND m.message_index = b.msg_index
		  JOIN LATERAL regexp_matches(
		...
		SELECT (SELECT amount FROM payout_total),
		       (SELECT amount FROM mfx_burned);
	`
